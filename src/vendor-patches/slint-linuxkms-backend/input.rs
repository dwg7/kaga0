                ...
                input::Event::Pointer(pointer_event) => {
                    match pointer_event {
                        input::event::PointerEvent::Motion(motion_event) => {
                            ...
                            // kaga addition: scale relative pointer motion by the
                            // display's resolution, 1080p as the baseline (unscaled)
                            // speed. Upstream applies libinput's dx/dy 1:1 to logical
                            // pixels regardless of resolution, so the same physical
                            // mouse movement crawls across a 4K panel (2x the linear
                            // distance of 1080p) at the same on-screen speed it had at
                            // 1080p -- this mirrors how desktop OSes scale pointer
                            // speed with display resolution.
                            let speed_scale = screen_size.height / 1080.0;
                            mouse_pos.x = (mouse_pos.x + motion_event.dx() as f32 * speed_scale)
                                .clamp(0., screen_size.width);
                            mouse_pos.y = (mouse_pos.y + motion_event.dy() as f32 * speed_scale)
