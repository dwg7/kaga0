...
## 監視スクリプトの落とし穴: `pgrep -f`の自己マッチ(2026-08-29)

バックグラウンドビルドの完了監視に`while pgrep -f "cargo build --release" >/dev/null; do sleep 60; done`
という形のループを使っていたが、これには**バグ**があった。監視ループ自体のコマンドライン
(`ps aux`に表示される`bash -c 'while pgrep -f "cargo build --release" ...`という文字列)に
`pgrep -f`の検索パターンが偶然含まれてしまうため、**ビルドが終了しても監視ループが
自分自身にマッチし続けて永遠に「まだ動いている」と誤判定する**。実際、libjpeg不足による
CMakeエラーで数時間前に終了していたビルドに、この誤判定のせいで気づけなかった
(手動確認で発覚)。

**教訓・今後の方針**:
- プロセス監視は`pgrep -f <文字列パターン>`ではなく、**起動直後に確保した実PIDに対する
  `kill -0 $PID`**で行う(数値比較なので自己マッチが原理的に起こらない)
- さらに確実にするため、今後のビルド起動は
  `nohup bash -c 'cargo build ...; echo "EXIT_CODE:$?" >> log' < /dev/null > log 2>&1 &`
